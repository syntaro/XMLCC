...
import jp.synthtarou.cceditor.midi.driver.CCDriverManager;
import jp.synthtarou.cceditor.midi.driver.CCDriver_UWP;
...
public class MXLIB01UWPMidi {
    ...
    static private void cbCallShortMessage(int device, int message) {
        CCDriver_UWP uwp = CCDriver_UWP._instance;
        CCDriverManager.receiveShort(uwp, device, message);
    }

    static private void cbCallLongMessage(int device, byte[] data) {
        CCDriver_UWP uwp = CCDriver_UWP._instance;
        CCDriverManager.receiveBytes(uwp, device, data);
    }
