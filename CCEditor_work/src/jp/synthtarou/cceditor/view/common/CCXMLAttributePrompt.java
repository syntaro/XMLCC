 ...
 */
public class CCXMLAttributePrompt extends javax.swing.JPanel implements IPromptForInput<CCWrapData<String>> {

    CCXMLNode _node;
    CCWrapData<String> _result;
    String _targetName;

    /**
     * Creates new form CCV13TextPrompt
     */
    public CCXMLAttributePrompt(CCXMLNode target, String attrName, String attrValue) {
        initComponents();
        _node = target;
        _targetName = attrName;
        _result = new CCWrapData<String>(attrName, attrValue);
    ...
    CCWrapDataList<String> _attrNameModel;

    protected void setAttrNameModel() {
        _attrNameModel = new CCWrapDataList<>();

        CCXMLTagRule tag = _node.getTagRule();
        if (tag != null) {
            List<CCXMLAttributeRule> attrRules = tag.listAttributes();
            for (CCXMLAttributeRule attr : attrRules) {
    ...
    CCWrapDataList<String> _attrValueModel;

    protected void setAttrValueModel(String attrName) {
        _attrValueModel = new CCWrapDataList<>();

        CCXMLTagRule tag = _node.getTagRule();
        if (tag != null) {
            CCXMLAttributeRule attr = tag.getAttribute(attrName);
            if (attr != null) {
    ...
    private void jButtonOKActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonOKActionPerformed
        _result = new CCWrapData<String>(getAttrName(), getAttrValue());
    ...
    @Override
    public CCWrapData<String> getPromptResult() {
        return _result;
    }
    ...
    public boolean validatePromptResult() {
        ...
        if (_targetName != null && name != null && _targetName.equalsIgnoreCase(name) == false) {
            CCXMLTagRule tagRule = _node.getTagRule();
