...
public class CCWrapDataList<T> extends ArrayList<CCWrapData<T>> implements ListModel, ComboBoxModel {
    ...
    public boolean addNameAndValue(String name, T value) {
        return add(new CCWrapData(name, value));
    }
    
    @Override
    public boolean add(CCWrapData<T> value) {
    ...
    public T readCombobox(JComboBox box) {
        ...
        CCWrapData<T> item = (CCWrapData)model.getSelectedItem();
        if (item != null) {
            return item.value;
        }
        return null;
    ...
    public void writeComboBox(JComboBox box, T value) {
        ...
        for (int i = 0; i < model.getSize(); ++ i) {
            CCWrapData<T> item = (CCWrapData)model.getElementAt(i);
            if (item != null) {
                if (value == null) {
                    if (item.value == null) {
