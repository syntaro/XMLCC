...
public class SMFPlayer {
    ...
    protected boolean sendProgramChangeBeforeNote() {
        ...
        while (pos < list.size()) {
            ...
            if (firstNote[channel] == null) {
                ...
                } else if (command == CCMIDICode.COMMAND_CONTROLCHANGE && data1 == 0) {
                    firstBank0[channel] = smf;
                } else if (command == CCMIDICode.COMMAND_CONTROLCHANGE && data1 == 32) {
                    firstBank32[channel] = smf;
                } else if (command == CCMIDICode.COMMAND_NOTEON) {
            ...
            while (!_breakSequence && list.get(pos)._tick <= elapsedTicks) {
                ...
                if (currentEvent.isBinaryMessage() || currentEvent.isMetaMessage()) {
                    ...
                    if (firstNote != null) {
                        if (firstNote[channel] != null) {
                            if (command == CCMIDICode.COMMAND_PROGRAMCHANGE
                                    || (command == CCMIDICode.COMMAND_CONTROLCHANGE && (data1 == 0 || data1 == 32))) {
                                continue;
                            } else if (command == CCMIDICode.COMMAND_NOTEON) {
    ...
    public void resetControllers() {
        if (getCurrentPosition() == 0 && !_paraPlay) {
            ...
            for (int i = start; i <= end; ++i) {
                SMFMessage message = new SMFMessage(0, CCMIDICode.COMMAND_CONTROLCHANGE + i, CCMIDICode.DATA1_CC_DAMPERPEDAL, 0);
                smfPlayNote(message);
                message = new SMFMessage(0, CCMIDICode.COMMAND_CONTROLCHANGE + i, CCMIDICode.DATA1_CC_ALLNOTEOFF, 127);
                smfPlayNote(message);
                message = new SMFMessage(0, CCMIDICode.COMMAND_CONTROLCHANGE + i, CCMIDICode.DATA1_CC_EXPRESSION, 127);
                smfPlayNote(message);
                message = new SMFMessage(0, CCMIDICode.COMMAND_CONTROLCHANGE + i, CCMIDICode.DATA1_CC_CHANNEL_VOLUME, 127);
                smfPlayNote(message);
            }
        }
    }
    ...
    public void allNoteOff() {
        ...
        for (int i = chFrom; i <= chTo; ++i) {
            SMFMessage message;
            message = new SMFMessage(0, CCMIDICode.COMMAND_CONTROLCHANGE + i, CCMIDICode.DATA1_CC_DAMPERPEDAL, 0);
            smfPlayNote(message);
            message = new SMFMessage(0, CCMIDICode.COMMAND_CONTROLCHANGE + i, CCMIDICode.DATA1_CC_ALLNOTEOFF, 127);
            smfPlayNote(message);
            message = new SMFMessage(0, CCMIDICode.COMMAND_CONTROLCHANGE + i, CCMIDICode.DATA1_CC_ALLSOUNDOFF, 127);
            smfPlayNote(message);
        }
    }
                ...
                if (_noteOnly) {
                    ...
                    switch(status & 0xf0) {
                        ...
                        case CCMIDICode.COMMAND_CONTROLCHANGE:
                            if (data1 == CCMIDICode.DATA1_CC_DAMPERPEDAL
                            || data1 == CCMIDICode.DATA1_CC_MODULATION
                            || data1 == CCMIDICode.DATA1_CC_ALLNOTEOFF
                            || data1 == CCMIDICode.DATA1_CC_ALLSOUNDOFF) {
                                skip = false;
                            }
                            break;
                    }
    ...
    public int getPositionOfFirstNote() {
        ...
        for (SMFMessage smf : list) {
            ...
            if ((command & 0xf0) == CCMIDICode.COMMAND_NOTEON) {
                return pos;
            }
            pos ++;
        }
