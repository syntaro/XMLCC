...
public class ConsoleElement {
    ...
    public String formatMessageDump() {
        switch (_type) {
            case TYPE_DWORD: {
                ...
                return CCUtilities.toHexFF(status) +" " + CCUtilities.toHexFF(data1) + " " + CCUtilities.toHexFF(data2);
            }
            case TYPE_DATA: {
                return CCUtilities.dumpHexFF(_data);
            }
        }
        return "Type=" + _type;
    }
    ...
    public static String toSegmentText(byte[] data) {
        return "[" + CCUtilities.dumpHexFF(data) + "]]";
    }
    ...
    public static String toSegmentText(int dword) {
        ...
        if (command == CCMIDICode.COMMAND_PITCHWHEEL) {
            return  "[Pitch " + channel + ":" + CCUtilities.toHexFF(data1) + CCUtilities.toHexFF(data2) +"=" + (data2 << 7 + data1) + "]";
        }
        ...
        if (command == CCMIDICode.COMMAND_CONTROLCHANGE) {
            ...
            return  "[CC-" + ccname + " " + channel + ":" + CCUtilities.toHexFF(data2) + "]";
        }

        return "****" + CCUtilities.toHexFF(status) + CCUtilities.toHexFF(data1) + CCUtilities.toHexFF(data2);
        //return  "[" + CCUtilitiesMidi.nameOfMessage(status, data1, data2) + " " + channel + "]";
    }
}
